...
def test_grant_role_success(mock_maritime_manager, mocker):
    ...
    tx_id = mock_maritime_manager.grant_role(
        sender_pk=VALID_TEST_PK,
        role_name="OEM",
        target_account_address="0x" + "b" * 40
    )
...
def test_grant_role_conflict(mock_maritime_manager, mocker):
    ...
    with pytest.raises(ValueError) as exc_info:
        mock_maritime_manager.grant_role(
            sender_pk=VALID_TEST_PK,
            role_name="OEM",
            target_account_address="0x" + "b" * 40
        )
...
def test_revoke_role_success(mock_maritime_manager, mocker):
    ...
    tx_id = mock_maritime_manager.revoke_role(
        sender_pk=VALID_TEST_PK,
        role_name="SERVICE",
        target_account="0x" + "c" * 40
    )
...
def test_log_service_event_success_as_operator(mock_maritime_manager, mocker):
    ...
    tx_id = mock_maritime_manager.log_service_event(
        ...
        service_protocol_hash="QmServiceHashOp"
    )
...
def test_get_part_id_success(mock_maritime_manager, mocker):
    ...
    part_id = mock_maritime_manager.get_part_id(
        serial_number="GEN123456",
...
def test_get_part_id_invalid_address_format(mock_maritime_manager):
    ...
    with pytest.raises(ValueError) as exc_info:
        mock_maritime_manager.get_part_id(
            serial_number="GEN123456",
            manufacturer_address="invalid-address"
        )
...
def test_get_part_id_address_encoder_error(mock_maritime_manager, mocker):
    ...
    mocker.patch(
        "src.app.maritime_manager.call_contract",
        side_effect=Exception("Value ... cannot be encoded by AddressEncoder")
    )

    with pytest.raises(ValueError) as excinfo:
        mock_maritime_manager.get_part_id(
            manufacturer_address="0x" + "a"*40,
            serial_number="SN123"
        )
...
def test_check_warranty_status_part_not_exists(mock_maritime_manager, mocker):
    ...
    mocker.patch(
        "src.app.maritime_manager.call_contract",
        side_effect=Exception("execution reverted: part not registered")
    )
...
def test_get_system_statistics_success(mock_maritime_manager, mocker):
    ...
    mocker.patch.object(mock_maritime_manager, 'check_warranty_status', side_effect=[
        (True, 100),
        (False, 0),
        (True, 200)
    ])
...
def test_get_system_statistics_critical_failure(mock_maritime_manager, mocker):
    ...
    mocker.patch.object(
        mock_maritime_manager,
        'get_all_parts',
        side_effect=Exception("Blockchain connection lost")
    )
